...
class HarvesterAnt(Ant):
    ...
    def action(self, gamestate):
        """Produce 1 additional 2 for the colony.
...
class ThrowerAnt(Ant):
    ...
    damage = 1
    food_cost = 3
    min_range = 0 #always equal 0
    max_range = float('inf') #the max range needs to take in account all spots (this is similar to interval notation)
    ...
    def nearest_bee(self):
        ...
        """
        place_tracker = self.place
        thedistance = 0
        while not place_tracker.is_hive:  ## while the position is not in Hive
            if self.min_range <= thedistance <= self.max_range and place_tracker.bees: #if distance is in between the range of the ANT'S Capability AND there is a bee present within range of the ANT'S throwing distance
                return random_bee(place_tracker.bees)
            else:
                place_tracker = place_tracker.entrance
                thedistance = thedistance + 1 #moves one spot up to keep checking
        return None
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if len(ant_place.bees) > 0:
            lst = list(ant_place.bees)
            for element in lst:
                if self.health <= 0:
                    Ant.reduce_health(element, amount + self.damage)
                else:
                    Ant.reduce_health(element, amount)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        lst = list(self_place.bees)
        if len(lst) > 0:
            if self.chew_countdown == 0:
                the_bee = random_bee(lst)
                Insect.reduce_health(the_bee, the_bee.health)
