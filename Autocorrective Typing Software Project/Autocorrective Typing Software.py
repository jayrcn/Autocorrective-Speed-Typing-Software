...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def contains_words(paragraphs):
        for words in split(lower(remove_punctuation(paragraphs))):
            if words in topic:
                return True
        return False
...
def accuracy(typed, reference):
    ...
    if len(typed_words) == 0 and len(reference_words) == 0:
        return 100.0
    return points / len(typed_words) * 100 if len(typed_words) else 0.0
    # END PROBLEM 3
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        return typed_word
    different_list = [diff_function(words, valid_words, limit) for words in valid_words]
    if min(diff_function) > limit:
        return typed_word
    else:
        return valid_words[different_list.index(min(different_list))]
...
def feline_flips(start, goal, limit):
    ...
    def helper_function(goal2, start2, counter):
        if counter > limit:
            return limit + 1
        if not goal2 and not start2:
            return counter
        elif not start2 or not goal2:
            return helper_function(start2[1:], goal2[1:], counter + 1)
        elif start2[0] == goal2[0]:
            return helper_function(start2[1:], goal2[1:], counter)
        else: 
            return helper_function(start2[1:], goal2[1:], counter + 1)
    return helper_function(start, goal, 0)
...
def minimum_mewtations(start, goal, limit):
    ...
    if not start or not goal:
        ...
        return minimum_mewtations(goal[1:], start[1:], limit)
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    for i in range(0, len(sofar)):
        if sofar[i] == prompt[i]:
            ...
            break
    upload({'id': user_id, 'progress': correct / len(prompt)})
